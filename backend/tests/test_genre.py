...
class GenreTestCase(unittest.TestCase):
    ...
    def test_read_all_genres(self):
        res = self.client().get(
            '/api/genres', headers=casting_assistant_headers)
    ...
    def test_read_single_genre(self):
        res = self.client().get(
            '/api/genres/1', headers=casting_assistant_headers)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        ...
        self.assertEqual(
            data['genre']['name'],
            self.test_genre_original.get('name'))

    def test_read_single_genre_error(self):
        res = self.client().get(
            '/api/genres/4004', headers=casting_assistant_headers)
        data = json.loads(res.data)
    ...
    def test_update_genre(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(
            data['genre']['name'], self.test_genre_update['name'])
        # test that getting id 1 will result in the new name
        res = self.client().get(
            '/api/genres/1', headers=casting_director_headers)
        data = json.loads(res.data)
        self.assertEqual(
            data['genre']['name'], self.test_genre_update['name'])
    ...
    def test_delete_single_genre(self):
        ...
        res = self.client().get(
            '/api/genres/2', headers=casting_director_headers)
        self.assertEqual(res.status_code, 200)
        # Now delete id 2
    ...
    def test_delete_single_genre_error(self):
        res = self.client().delete(
            '/api/genres/9999', headers=casting_director_headers)
        self.assertEqual(res.status_code, 404)
